...
open class YDCameraManager: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate{
    ...
    open var doZoomAction: ((_ zoom: CGFloat) -> Void)?
    ...
    open var torchMode: CameraTorchMode = .off {
        didSet {
            if cameraIsSetup && torchMode != oldValue {
                _updateCameraTorchMode(torchMode)
            }
        }
    }
    ...
    open func changeTorchMode() -> CameraTorchMode {
        guard let newTorchMode = CameraTorchMode(rawValue: (torchMode.rawValue + 1) % 2) else { return torchMode }
    ...
    fileprivate func attachZoom(_ view: UIView) {
        DispatchQueue.main.async {
            self.zoomGesture.addTarget(self, action: #selector(YDCameraManager._zoomStart(_:)))
    ...
    fileprivate func attachFocus(_ view: UIView) {
        DispatchQueue.main.async {
            self.focusGesture.addTarget(self, action: #selector(YDCameraManager._focusStart(_:)))
    ...
    fileprivate func attachExposure(_ view: UIView) {
        DispatchQueue.main.async {
            self.exposureGesture.addTarget(self, action: #selector(YDCameraManager._exposureStart(_:)))
    ...
    fileprivate func _setupCamera(_ completion: @escaping () -> Void) {
        ...
        sessionQueue.async {
            if let validCaptureSession = self.captureSession {
                ...
                self._updateDeviceTorch(self.torchMode)
...
//        }
    ...
    open func _doFlipAnimation() {
        ...
        if let validEmbeddingView = embeddingView,
            ...
            if YDCameraManager._blurSupported() {
    ...
    fileprivate func _updateCameraTorchMode(_ mode: CameraTorchMode) {
        _updateDeviceTorch(mode)
    }
    ...
    fileprivate func _updateDeviceTorch(_: CameraTorchMode) {
        captureSession?.beginConfiguration()
        defer { captureSession?.commitConfiguration() }
        for captureDevice in AVCaptureDevice.videoDevices {
            guard let avTorchMode = AVCaptureDevice.TorchMode(rawValue: torchMode.rawValue) else { continue }
...
extension YDCameraManager: AVCaptureMetadataOutputObjectsDelegate {
    /**
